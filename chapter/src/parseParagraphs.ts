...
export async function parseParagraphs(
  ...
  const paragraphPromises = Array.from(
    ...
    (paragraph): Paragraph => {
      ...
      para.verseIds = Array.from(paragraph.children)
        .map(
          (child): string => {
            // if (child.id === 'closing1') {
            // console.log(child);
            // }
            return child.id;
          },
      ...
      para.classList = paragraph.className.split(' ');
      para.nodeName = paragraph.nodeName;
